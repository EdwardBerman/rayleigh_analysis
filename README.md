...
3. You can run the training via `python3 -m external.custom_hermes.train dataset=heat backbone=hermes`, this is also in `mesh.sh`. That being said, we use the default Checkpoints supplied from the original Hermes paper for evaluation
4. Models are evaluated using `eval_mesh.sh` or `python3 -m external.custom_hermes.eval_rollout dataset=heat backbone=hermes model_save_path=model_checkpoints/[model pt file]`
