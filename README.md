...
3. To train the equivalent(ish) Seperable Unitary GCN on the heat diffusion data on graph, do `python3 -m toy_heat_diffusion.train --data_dir toy_heat_diffusion/data --start_time 0.0 --train_steps 5 --eval_steps 2 --model seperable_unitary --layers 12 --hidden 128 --epochs 200 --act GroupSort`. For Lie Unitary GCN, simply change the `--model` flag to `lie_unitary`
