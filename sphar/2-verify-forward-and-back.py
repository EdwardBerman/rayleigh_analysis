...
def plot_global_latlon_e2e(
    latlon_data, lat, lon, hp_map, latlon_back, nside,
):

    lon2d, lat2d = np.meshgrid(lon, lat)
    hp_lat, hp_lon = get_latlons_for_healpix(nside)

    vmin = float(np.nanpercentile(latlon_data, 0))
    vmax = float(np.nanpercentile(latlon_data, 100))

    fig = plt.figure(figsize=(24, 6))
    gs = gridspec.GridSpec(1, 4, width_ratios=[1, 1, 1, 0.04], wspace=0.08)

    ax_ll = fig.add_subplot(gs[0], projection=ccrs.PlateCarree())
    ax_hp = fig.add_subplot(gs[1], projection=ccrs.PlateCarree())
    ax_ll_back = fig.add_subplot(gs[2], projection=ccrs.PlateCarree())
    ax_cb = fig.add_subplot(gs[3])

    for ax in (ax_ll, ax_hp, ax_ll_back):
    ...
    im = ax_ll_back.pcolormesh(lon2d, lat2d, latlon_back.T, transform=ccrs.PlateCarree(
    ), cmap="RdBu_r", vmin=vmin, vmax=vmax, shading="nearest")

    ax_ll_back.set_title("LatLon (back)")
...
latlon_backmap = train._project_to_latlon(hp_map)[0]
