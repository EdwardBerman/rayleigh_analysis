...
def clusterize(pointcloud: np.ndarray, max_cluster_size: int, init_cluster_centers=None, verbose=True):
    """
    pointcloud: [N, D] np.float32
    max_cluster_size: maximum points per cluster

    Returns:
        labels: [N] np.int64
        centers: [K, D] np.float32
    """
    pointcloud = pointcloud.astype(np.float32)
    # Same heuristic you use in main (can tweak the +1 if you want)
    n_clusters = int(np.ceil(pointcloud.shape[0] / max_cluster_size)) + 1
    if verbose:
        print(f"Clustering {pointcloud.shape[0]} points into {n_clusters} clusters (max size {max_cluster_size})")

    labels, centers = constrained_clustering_numpy(
        pointcloud,
