...
def main(cfg):
    ...
    for split, dataset in datasets_dict.items():
        ...
        for mesh_idx, v in results["losses"].items():
            ...
            plt.fill_between(t, true_rq.mean(axis=0) - true_rq_std, true_rq.mean(axis=0) + true_rq_std, color="blue", alpha=0.3, label="Std Dev Ground Truth RQ")
            plt.fill_between(t, pred_rq.mean(axis=0) - pred_rq_std, pred_rq.mean(axis=0) + pred_rq_std, color="orange", alpha=0.3, label="Std Dev Prediction RQ")
