...
from iresreg.external.torch_scatter import scatter_sum
...
class DMPNNEncoder(nn.Module):
    def __init__(self, hidden_size, node_fdim, edge_fdim, depth, batch_norm, dropout):
        ...
        self.H1 = nn.Linear(hidden_size, hidden_size, bias=True)
        self.H2 = nn.Linear(hidden_size, 1, bias=True)
    ...
    def forward(self, data):
        ...
        pooled = global_mean_pool(node_attr, batch)

        if self.batch_norm:
            pooled = self.bn(pooled)

        mid = self.act_func(self.dropout(self.H1(pooled)))
        out = self.H2(mid)
