...
class OrthogonalGCNConvLayer(nn.Module):
    def __init__(self, dim_in, dim_out, dropout=0.0, residual=False,
                 global_bias=True, T=10, adaptive=False, use_hermitian=False,
    ...
    def forward(self, batch):
        ...
        batch.x = self.model(batch.x, batch.edge_index, batch.edge_weight) if hasattr(
            batch, 'edge_weight') else self.model(batch.x, batch.edge_index)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.lin.weight.upper_triangular_params = in_channels * \
            (in_channels-1)//2  # used for counting the number of parameters
...
class TaylorGCNConv(MessagePassing):
    def __init__(
        ...
        **kwargs
        ...
        if adaptive:
            self.term_weights = nn.Parameter(torch.zeros(T))
    ...
    def forward(self, x: Tensor, edge_index: Adj, edge_weight: OptTensor = None, **kwargs) -> Tensor:
        ...
        if not self.adaptive:
            ...
            weights = torch.softmax(self.term_weights, dim=0)  # (T,) sums to 1

            # partial_sums[i] = sum of first (i+1) x_k terms
            partial_sums = []
            running = x.clone()
            for k in range(self.T):
                x_k = self.conv(x_k, edge_index, edge_weight,
                                apply_feature_lin=False, **kwargs) / (k+1)
                running = running + x_k
                partial_sums.append(running)  # each is (N, d)

            # weighted sum of partial sums: sum_i w_i * partial_sums[i]
            out = sum(weights[i] * partial_sums[i] for i in range(self.T))
            return out
