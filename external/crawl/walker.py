...
class Walker(torch.nn.Module):
    ...
    def __init__(self, steps, win_size, train_start_ratio, compute_id_feat=True, compute_adj_feat=True, walk_delta=0.0):
    ...
    def sample_start(start_p, graph_idx, graph_offset, order, device):
        ...
        idx = graph_offset + \
            (torch.randint(0, MAXINT, (num_graphs,), device=device) % order)
        idx = torch.cat([idx, torch.randperm(
            num_nodes, device=device)[:num_extra]])
    ...
    def unweighted_choice(self, i, walks, adj_nodes, adj_offset, degrees, nb_degrees, choices):
        ...
        if self.non_backtracking and i > 0:
            ...
            if bt.max():
                bt_nodes = walks[i][bt]
                chosen_edges[bt] = adj_offset[bt_nodes] + (edge_idx[bt] + 1 + (
                    choices[i][bt] % nb_degrees[bt_nodes])) % degrees[bt_nodes]
    ...
    def sample_walks(self, data, steps=None, start_p=1.0):
        ...
        nb_degrees = nb_degree_mask * degrees + \
            (~nb_degree_mask) * (degrees - 1)

        for i in range(steps):
            chosen_edges = self.unweighted_choice(
                i, walks, adj_nodes, adj_offset, degrees, nb_degrees, choices)
            ...
            if self.compute_adj:
                ...
                edges[i + 1, s - o:] = (torch.gather(adj_bits[prev], 2, cur_int).view(
                    o, -1) >> (cur_id % 63).view(1, -1)) % 2 == 1
