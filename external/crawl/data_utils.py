...
def preproc(data):
    ...
    if edge_idx.shape[1] > 0 and not pygeo.utils.is_undirected(edge_idx):
        ...
        e = edge_idx.detach().numpy()
        x = [x_map[(e[0, i], e[1, i])] if (e[0, i], e[1, i]) in x_map.keys(
        ) else x_map[(e[1, i], e[0, i])] for i in range(e.shape[1])]
    ...
    bit_id[data.node_id, data.node_id //
           63] = torch.tensor(1) << data.node_id % 63
    data.adj_bits = scatter_sum(
        bit_id[edge_idx[0]], edge_idx[1], dim=0, dim_size=data.num_nodes)

    # compute node offsets in the adjacency list
    data.degrees = pygeo.utils.degree(
        edge_idx[0], dtype=torch.int64, num_nodes=data.num_nodes)
...
def merge_batch(graph_data):
    ...
    node_graph_idx = torch.cat(
        [i * torch.ones(x, dtype=torch.int64) for i, x in enumerate(num_nodes)])
    edge_graph_idx = torch.cat(
        [i * torch.ones(x, dtype=torch.int64) for i, x in enumerate(num_edges)])
    ...
    adj_bits = torch.cat([F.pad(
        b, (0, max_enc_length-b.shape[1], 0, 0), 'constant', 0) for b in adj_bits], dim=0)
...
class CRaWlLoader(DataLoader):
    ...
    def __init__(self, dataset, batch_size=1, shuffle=False, **kwargs):
        super(CRaWlLoader, self).__init__(dataset, batch_size,
                                          shuffle, collate_fn=merge_batch, **kwargs)
